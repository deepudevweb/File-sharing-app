...
function UploadForm({ uploadBtnClick, progress }) {
    ...
    return (
        <div className='text-center'>
            ...
            {file ? <FilePreview removeFile={() => setFile(null)} file={file} /> : null}

            {progress > 0 ? <Progressbar progress={progress} /> : <button disabled={!file} className='p-2 text-white bg-primary w-[30%] m-5 rounded-full disabled:bg-gray-400' onClick={() => uploadBtnClick(file)}>Upload</button>}
        </div>
    )
}
